...
class AuthTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testConstructor() {
        ...
        try {
            ...
            $this->assertInternalType('string', $e->getMessage(),
                "[-] Auth() must throw an exception if error called when session not instantiated"
            );
        }
        ...
        try {
            ...
            $this->assertInternalType('string', $e->getMessage(),
                "[-] Auth() must throw an exception if error called when DI not configured"
            );
        }
    ...
    public function testDestroyedSession() {
        ...
        // check return if not started
        session_destroy();
        $session = $this->invokeMethod($this->auth, 'hasSession', array());

        $this->assertTrue($session,
            "[-] If session is destroyed, hasSession() must start new and return true"
        );
    }
}